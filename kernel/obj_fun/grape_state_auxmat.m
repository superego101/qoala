...
%
% NOTE: This function is loosely based on the Spinach (Goodwin, Kuprov) 
%       function grape(), and should be referenced accordingly.

function [data,fdty,grad]=grape_state_auxmat(param,drift_sys,ctrls,amps,wf,init,targ)
...
auxmat_method=param.auxmat_method;
    ...
    if n_outputs>2
        ...
        for n = 1:nsteps
            ...
            for k=1:kctrls
                ...
                aux_vector=propagate(param.space,aux_mat,dt(n),auxmat_method,prop_zeroed,aux_vec);
    ...
    if n_outputs>2
        ...
        for n = 1:nsteps
            ...
            for k=1:kctrls
                ...
                % Propagate the auxiliary vector
                aux_mat=[L, ctrls{k}; 0*L, L];
                prop_dirdiff=propagate(param.space,aux_mat,dt(n),auxmat_method,min([prop_zeroed 1e-14]));
...
if strcmp(param.space,'liouville')
    ...
    if n_outputs>2
        ...
        for n = nsteps:-1:1
            ...
            for k=1:kctrls
                grad(n,k) = amps(k)*real(bwd_trajn'*dP_rho{k}(:,n));
