...
function  [data,fdty,grad]=grape_state_qoala(param,drift_sys,ctrls,amps,wf,init,targ)
...
elseif n_outputs==3
    ...
    if drift_sys.adapt_counter < drift_sys.adapt_minit
        ...
        if ismember(adapt_method,{'exact'})
            ...
            % pull the final state/propagator and normalise
        ...
        if ismember(adapt_method,{'exact'}), error_f=error_f*abs(1-real(rho_est'*rho_0)); end
        ...
        while (error_f > tol_f)
            ...
            error_f=abs(1-real(rho_est'*rho_0));
    ...
    for n = nsteps:-1:1
        ...
        for k=1:kctrls
            ...
            for i=1:num_a
                ...
                grad_col(k)=grad_col(k)+amps(k)*real(bwd_split{num_b-i}'*rho_n);
            end
