...
% in the specified direction. Syntax:
%
%  [alpha,fx_1,gfx_1,exitflag,data]=...
%          fmaxlinesearch(param,cost_function,d_0,x_0,fx_0,gfx_0,data)
%
% Parameters:
%
...
function [alpha,fx_1,gfx_1,exitflag,data,param]=...
    fmaxlinesearch(param,cost_function,d_0,x_0,fx_0,gfx_0,data)
...
% Step length guess
if strcmp(param.method,'lbfgs')&&(data.count.iter==1)
...
% find if any objectives were adaptive
if data.count.iter~=1
    if isfield(param,'drift_sys')
        if iscell(param.drift_sys)
            for k=1:numel(param.drift_sys)
                if isfield(param.drift_sys{k},'adapt') &&...
                        isfield(param.drift_sys{k},'adapt_counter') &&...
                        isfield(param.drift_sys{k},'adapt_minit')
                    if param.drift_sys{k}.adapt &&...
                            ~(param.drift_sys{k}.adapt_counter<param.drift_sys{k}.adapt_minit)
                        adapt=true;
                    end
                end
            end
            ...
            if isfield(param.drift_sys,'adapt') &&...
                    isfield(param.drift_sys,'adapt_counter') &&...
                    isfield(param.drift_sys,'adapt_minit')
                if param.drift_sys.adapt &&...
                        ~(param.drift_sys.adapt_counter<param.drift_sys.adapt_minit)
                    adapt=true;
                end
            end
        end
...
if adapt
    ...
    alpha=1;
    [data,param,fx_1,gfx_1]=objective(x_0+alpha*d_0,cost_function,data,param);
    exitflag=0;
else
    ...
    [A,B,alpha,fx_1,gfx_1,exitflag,data]=bracketing(cost_function,alpha,d_0,x_0,...
        fx_0,gfx_0,fx_1,gfx_1,data,param);
...
if isfield(param,'drift_sys')
    if iscell(param.drift_sys)
        for k=1:numel(param.drift_sys)
            if isfield(param.drift_sys{k},'adapt')
                param.drift_sys{k}.adapt=false;
            end
        end

    else
        if isfield(param.drift_sys,'adapt')
            param.drift_sys.adapt=false;
        end
    end
end
...
while true
    ...
    if (~alpha_conditions(1,alpha,fx_0,fx_2,gfx_0,[],d_0,param))||...
       (~alpha_conditions(0,[],fx_0,fx_2,[],[],[],param))
    ...
    if alpha_conditions(2,alpha,fx_0,fx_2,gfx_0,gfx_2,d_0,param)
    ...
    if ~alpha_conditions(3,[],[],[],[],gfx_2,d_0,param)
    ...
    brcktEndpntB = alpha_2+param.ls_tau1*(alpha_2-alpha_1);
    ...
    [data,~,fx_2,gfx_2]=objective(x_1,cost_function,data,param);
end
...
while true
    ...
    End_A = A.alpha + min(param.ls_tau2, param.ls_c2)*(B.alpha - A.alpha);
    End_B = B.alpha - param.ls_tau3*(B.alpha - A.alpha);
    ...
    [data,~,fx_1,gfx_1]=objective(x_0+alpha*d_0,cost_function,data,param);
    
    % Store current bracket position of A
    Tmp=A;
    
    % Update the current brackets
    if (~alpha_conditions(1,alpha,fx_0,fx_1,gfx_0,gfx_1,d_0,param))||...
       (~alpha_conditions(0,alpha,A.fx,fx_1,A.gfx,gfx_1,d_0,param))
        ...
        if alpha_conditions(2,alpha,fx_0,fx_1,gfx_0,gfx_1,d_0,param)
            exitflag = []; return;
        end
...
function test=alpha_conditions(test_type,alpha,fx_0,fx_1,gfx_0,gfx_1,d_0,param)
...
elseif test_type==1
    ...
    test=(fx_1 >= fx_0 + param.ls_c1*alpha*(gfx_0'*d_0));
    
elseif test_type==2
    
    % Strong Wolfe (Curvature) condition - avoid underestimated step length
    test=(abs(gfx_1'*d_0) <= param.ls_c2*abs(gfx_0'*d_0));
