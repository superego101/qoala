...
function optimpulse=state2state_xy(varargin)

options=struct(varargin{:});
if      ~isfield(options,'omega'),          error('must provide ''omega'' as an input')
elseif  ~isfield(options,'initial'),        error('must provide ''initial'' as an input')
...
rho_init=options.initial;   options=rmfield(options,'initial');
rho_targ=options.target;    options=rmfield(options,'target');
...
% sparse, normalised, initial and target states
rho_init=sparse(rho_init/norm(full(rho_init)));
rho_targ=sparse(rho_targ/norm(full(rho_targ)));

% Define control parameters
options.space='liouville';                % state space
options.basis='sphten';                   % basis set
options.operators=H_ss;
options.optimcon_fun=@grape_state_qoala;  % Optimal control function
options.drift_sys.interaction=H_int;      % Drift without offsets
options.drift_sys.offset=2*pi*omega(:);   % Drift without coupling
options.rho_init={rho_init};              % Initial state
options.rho_targ={rho_targ};              % Target state
